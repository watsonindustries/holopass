		...
		if ($error) {
			...
			switch ($error.code) {
				...
				case $error.TIMEOUT:
					...
					break;
				default:
					errMsg = "An unknown error occurred while trying to get your location. Please try again later!";
					break;
			}
			nearbyEventData = { error: errMsg };
			pause();
...
>
	<div
		id="check-in-card"
		class="card bg-slate-50 p-4 shadow-lg"
	>
						...
						{#if nearbyEventData}
							{#if nearbyEventData.error}
								<PopupStatusCard
									...
									text_color="white"
								>
									{nearbyEventData.error?.endsWith(".") ? nearbyEventData.error : `${nearbyEventData.error}.`}
								</PopupStatusCard>
							{:else}
								{#if nearbyEventData.badges}
										...
										{#if badgeData}
											{#if badgeData.error}
												<PopupStatusCard
													...
													text_color="white"
												>
													Failed to fetch badges: {badgeData.error?.endsWith(".") ? badgeData.error : `${badgeData.error}.`}
												</PopupStatusCard>
											{:else}
												{#if badgeData.badges}
													<div class="text-lg">Looking to check in for an event badge? We found the following events near you happening now!</div>
													<div class="p-2" />
