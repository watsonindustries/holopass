...
import { loadBadge, loadBadgeLocation, loadNearestBadgesTemporal, loadProfile } from '../../../supabase';
...
export async function POST({ request, locals: { supabase, getSession } }) {
	...
	const activeNearbyBadges = await loadNearestBadgesTemporal(
		supabase, loc[0], loc[1], 1600 /* 1-mile radius */
	);
	...
	const { err } = await supabase.from('profiles')
		.update({
